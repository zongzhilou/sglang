...
class ShensiAttentionResidual(nn.Module):
    ...
    def _proj_weight(self) -> torch.Tensor:
        if self._fused_proj is None:
            fused = torch.cat(
                [
                    p.detach().reshape(p.shape[0], -1).float()
                    for p in (self.gate_proj.weight, self.k_proj, self.q_proj)
                ],
                dim=0,
            ).contiguous()
    ...
    def forward(
        ...
        if (
            ...
            and (delta is None or delta.is_contiguous())
        ):
            x, rs = _attn_res_prenorm(prefix_sum, delta, self.norm.eps)
            proj = F.linear(x, self._proj_weight())
            updated = _attn_res_delta(proj, rs, prefix_sum, delta, self.gate_proj.bias)
            if num_blocks > 0:
            ...
            # One 5H projection of the state drives the gates, the write direction and the query.
            proj = F.linear(state, self._proj_weight())
            gate = proj[..., : 3 * h] + self.gate_proj.bias.float()
            decay, erase, write = torch.sigmoid(
                gate.reshape(*state.shape[:-1], 3, -1)
            ).unbind(-2)

            # Decay the prefix, project out the erased direction, then write the delta.
            khat = F.normalize(proj[..., 3 * h : 4 * h], dim=-1)
            forgotten = decay * prefix
            r = (khat * erase * forgotten).sum(dim=-1, keepdim=True)
            updated = (
                forgotten - khat * r + write * (delta if delta is not None else 0.0)
            )

            if num_blocks > 0:
                query = proj[..., 4 * h : 5 * h]
                bank = blocks[..., :num_blocks, :].float()
        ...
        output = output.to(prefix_sum.dtype)
        if block_write_idx >= 0:
            written = output.to(blocks.dtype).unsqueeze(-2)
            blocks = torch.cat(
                [
        ...
        return output, updated.to(prefix_sum.dtype), blocks
...
class ShensiDecoderLayer(DeepseekV4DecoderLayer):
    ...
    def forward(
        ...
        hidden_states, prefix_sum, residual = self.self_attention_attn_res(
            ...
            block_write_idx=self.prev_valid_blocks if self.is_block_write_layer else -1,
        )
